...
import { signupLoading, signupSuccess, signupError } from "../redux/actions";
...
export const Signup = () => {
  ...
  let handleSignup = (e) => {
    ...
    if (name === "") {
      setAlertContent("Please enter the name.");
      setAlert(true);
      setSeverity("warning");
    } else if (!emailValid) {
      setAlertContent("Pealse enter a valid Email.");
      setAlert(true);
      setSeverity("warning");
    } else if (mobile_number.length !== 10) {
      setAlertContent("Please enter a valid mobile number.");
      setAlert(true);
      setSeverity("warning");
    } else if (password.length <= 8) {
      setAlertContent("Password must be atleast of eight characters.");
      setAlert(true);
      setSeverity("warning");
    } else {
      axios({
        method: "post",
        url: "http://localhost:7000/usersignup",
        data: {
          name: name,
          email: email,
          ...
          >
            <Button
              sx={{
                ...
                "@media (max-width:768px)": { fontSize: "15px", width: "80%" },
                "@media (max-width:375px)": { fontSize: "15px", width: "80%" },
              }}
          ...
          >
            <Button
              sx={{
                ...
                "@media (max-width:768px)": { fontSize: "12px", width: "80%" },
                "@media (max-width:375px)": { fontSize: "12px", width: "80%" },
              }}
            >
              <img
                className="signup_googleimg"
                alt=""
